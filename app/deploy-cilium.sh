...
# In case of installing Cilium for a specific Kubernetes version, the --kubernetes-version vx.y.z parameter can be appended to the minikube start command for bootstrapping the local cluster.
# By default, minikube will install the most recent version of Kubernetes
minikube start --profile=minikube --network-plugin=cni  --memory=4096 --vm-driver=none #the none driver, the kubectl config and credentials generated are owned by root in the root user’s home directory
#minikube start --profile=minikube --network-plugin=cni  --memory=4096 --vm-driver=none --kubernetes-version=v$KUBERNETES_VERSION #the none driver, the kubectl config and credentials generated are owned by root in the root user’s home directory
...
#Install Cilium as DaemonSet into your new Kubernetes cluster.
#The DaemonSet will automatically install itself as Kubernetes CNI plugin
#quick-install.yaml is a pre-rendered Cilium chart template
...
echo echo "Waiting for the connectivity test to be ready ..."
for i in {1..60}; do # Timeout after 3 minutes, 60x5=300 secs
     if kubectl get pods --namespace=kube-system  | grep ContainerCreating ; then
         sleep 5
     else
